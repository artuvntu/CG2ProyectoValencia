...
void Primitivas::prismaEstandar(unsigned int typeText, double x, double y, double z, int fondo, int tapa){
    unsigned char tActual;
    double distanciaText[2];
    tActual = (typeText>>20) & 0xF;
	distanciaText[0] = this->textParedTam[tActual] * x / 10 + this->textParedInicioX[tActual];
	distanciaText[1] = this->textParedTam[tActual] * y / 10 + this->textParedInicioY[tActual];
    glBindTexture(GL_TEXTURE_2D, this->textPared[tActual]);
    glNormal3d(0, 0, -1);
    glBegin(GL_POLYGON);
    glTexCoord2d(this->textParedInicioX[tActual], this->textParedInicioY[tActual]);
    glVertex3d(0, 0, 0);
    glTexCoord2d(distanciaText[0], this->textParedInicioY[tActual]);
    glVertex3d(x, 0, 0);
    glTexCoord2d(distanciaText[0], distanciaText[1]);
    glVertex3d(x, y, 0);
    glTexCoord2d(this->textParedInicioX[tActual], distanciaText[1]);
    glVertex3d(0, y, 0);
    glEnd();
