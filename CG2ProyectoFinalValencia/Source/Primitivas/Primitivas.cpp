...
void Primitivas::setTamTextPared(int cantidad, ...){
    va_list argumentos;
    va_start(argumentos, cantidad);
    if (cantidad > 6) cantidad = 6;
    for (int i = 0; i<cantidad; i++) this->tamTextPared[i] = va_arg(argumentos, int);
    va_end(argumentos);
}
void Primitivas::resetTamTextPared(){
    for (int i = 0; i<6; i++) this->tamTextPared[i] = 1;
}
void Primitivas::prismaEstandar(unsigned int typeText, double x, double y, double z){
    ...
    glTexCoord2d(0, 0);
    glVertex3d(0, 0, 0);
    glTexCoord2d(this->tamTextPared[tActual], 0);
    glVertex3d(x, 0, 0);
    glTexCoord2d(this->tamTextPared[tActual], this->tamTextPared[tActual]);
    glVertex3d(x, y, 0);
    glTexCoord2d(0, this->tamTextPared[tActual]);
    glVertex3d(0, y, 0);
    glEnd();
